...
export default function Contact({ className }: { className?: string }) {
    return (
        <div className={`${className} space-y-6`}>
            <Paragraph>
                Want to get in touch with us? Send us a message over on{" "}
                <b>Facebook</b>, <b>Discord</b> or{" "}
                <b>
                    email us at{" "}
                    <a
                        href="mailto:contact@csclub.org.au"
                        className="underline"
                    >
                ...
                <br />
                <br />
                To keep up to date with our news and events, check out our
                Instagram!
            </Paragraph>
            <div className="flex flex-wrap justify-start gap-4 text-3xl md:text-5xl">
                {LINKS.map((link, i) => (
                    <Link
                        {...link}
                        borderColour={LINK_BORDER_COLOURS[link.name]}
                        key={i}
                    />
                ))}
            </div>
